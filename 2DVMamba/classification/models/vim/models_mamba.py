...
class Block(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.mixer(hidden_states, inference_params=inference_params, HH=HH, WW=WW)
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        self.use_cross_cls_token = use_cross_cls_token
        ...
        if if_cls_token:
            if use_cross_cls_token:
                self.num_tokens = img_size // patch_size * 2 - 1
                self.cls_token = nn.Parameter(torch.zeros(1, self.num_tokens, self.embed_dim))
            else:
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.if_cls_token:
            if self.use_cross_cls_token:
                x = x.reshape(B, HH, WW, DD)
                x_cls = torch.zeros(B, HH + 1, WW + 1, DD, dtype=x.dtype, device=x.device)
                x_cls[:, 0:HH//2, 0:WW//2, :] += x[:, 0:HH//2, 0:WW//2, :]
                x_cls[:, HH//2 + 1:, 0:WW//2, :] += x[:, HH//2:, 0:WW//2, :]
                x_cls[:, 0:HH // 2, WW // 2 + 1:, :] += x[:, 0:HH // 2, WW // 2:, :]
                x_cls[:, HH // 2 + 1:, WW // 2 + 1, :] += x[:, HH // 2:, WW // 2:, :]

                cls_token = self.cls_token.expand(B, -1, -1)
                t = WW//2
                x_cls[:, HH//2, 0:WW//2, :] += cls_token[:, 0:t, :]
                x_cls[:, HH // 2, WW // 2 + 1:, :] += cls_token[:, t:2 * t, :]
                x_cls[:, 0:HH // 2, WW // 2, :] += cls_token[:, 2 * t:3 * t, :]
                x_cls[:, HH // 2 + 1 :, WW // 2, :] += cls_token[:, 3 * t:4 * t, :]

                x_cls[:, HH // 2, WW // 2, :] += cls_token[:, -1, :]
        ...
        if self.if_cls_token:
            if self.use_cross_cls_token:
                hidden_states = hidden_states.reshape(B, HH + 1, WW + 1, -1)
                return hidden_states[:, HH // 2 + 1, WW // 2 + 1, :]
            else:
                raise NotImplementedError
...
@register_model
def vim_tiny_2d_cross_cls(pretrained=False, **kwargs):
    model = VisionMamba(
        ...
        bimamba_type="v2", if_cls_token=True, if_divide_out=True,
        use_cross_cls_token=True, **kwargs)
